...
def deleteproduct(id):
    category = Addproduct.query.get_or_404(id)
    db.session.delete(category)
    flash(f"The brand {category.name} was deleted from your database","success")
    db.session.commit()
    return redirect(url_for('admin'))
...
def pageabout():
    return render_template('about/about.html',brands=brands(),categories=categories())


@app.errorhandler(404)
def page_not_found(error):
    return (render_template("error/error.html", error="Página no encontrada"),
            404)    